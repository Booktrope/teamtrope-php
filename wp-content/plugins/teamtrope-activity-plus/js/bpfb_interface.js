...
var BpfbPhotoHandler = function () {
	...
	var createMarkup = function () {
		...
		var uploader = new qq.FileUploader({
			...
			"allowedExtensions": ['jpg', 'jpeg', 'gif','doc','docx','pdf','png'],
			"action": ajaxurl,
			"params": {
	...
	var createRemoteImagePreview = function () {
		...
		$.post(ajaxurl, {"action":"bpfb_preview_remote_image", "data":imgs}, function (data) {
			var html = '';
			$.each(data, function() {
				html += '<img class="bpfb_preview_photo_item" src="' + this + '" width="180px" />' +
	...
	var createPhotoPreview = function (id, fileName, resp) {
		...
		var html = '<img class="bpfb_preview_photo_item" src="' + _bpfbTempImageUrl + resp.file + '" width="180px" />' +
