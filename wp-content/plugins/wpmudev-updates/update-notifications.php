    ...
    function handle_dismiss() {
        ...
        if (isset($_REQUEST['dismiss-release'])) {
            ...
            function auto_install_url($project_id) {
                ...
                $local_projects = $this->get_local_projects();
                $api_key = $this->get_apikey();
                if (!isset($local_projects[$project_id]) && isset($data['projects'][$project_id]['autoupdate']) && $data['projects'][$project_id]['autoupdate'] == 1 && $api_key && $this->user_can_install($project_id) && (isset($data['downloads']) && $data['downloads'] == 'enabled')) {
        ...
        <div id="wpdv-dash-links">
            ...
            <?php if ($count) { ?>
                <a class="quick-link" id="ql-updates" href="<?php echo $this->updates_url; ?>" title="<?php _e('Update your WPMU DEV Plugins and Themes', 'wpmudev'); ?>"><?php _e('Updates', 'wpmudev');
            echo $count_output;
                ?></a>
        <?php } ?>
