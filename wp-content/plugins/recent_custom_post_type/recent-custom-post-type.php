...
class RcptWidget extends WP_Widget {
	...
	function RcptWidget () {
		...
		parent::WP_Widget('rcpt', 'Recent Custom Posts Type Widget', $widget_ops);
	}
	...
	function form ($instance) {
		...
		if ($_fields) foreach ($_fields as $field) {
			if (preg_filter($skips, ':skip:', $field)) continue;
...
EORcptJs;
	...
	function widget ($args, $instance) {
		...
		if ($post_author) $query_args['author'] = $post_author;
		if ($featured_image) $query_args['meta_key'] = '_thumbnail_id';
		...
		if ($query->have_posts()) {
			...
			while ($query->have_posts()) {
				...
				foreach ($post_fields as $field => $value) {
					if (!in_array($field, array_keys($fields))) continue; // Not here
					if (!$fields[$field]) continue; // No label
