...
class BP_MAM_Settings {
	 ...
	 */
	function bp_mam_settings( $id = null ) {
		global $wpdb, $bp;
	...
	function populate() {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp->mam->bp_mam_settings} WHERE id = %d", $this->id ) ) ) {
	...
	function save() {
		...
		do_action( 'bp_mam_data_before_save', $this );
						
		if ( $this->id ) {
			// Update
			$result = $wpdb->query( $wpdb->prepare( 
					"UPDATE {$bp->mam->bp_mam_settings} SET 
						field_1 = %d,
						field_2 = %d,
						field_3 = %d
					WHERE id = %d",
			...
			// Save
			$result = $wpdb->query( $wpdb->prepare( 
					"INSERT INTO {$bp->upc->bp_mam_settings} ( 
						field_1,
						field_2,
						field_3 
					) VALUES ( 
	...
	function delete() {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$bp->mam->bp_mam_settings} WHERE id = %d", $this->id ) );
	}
