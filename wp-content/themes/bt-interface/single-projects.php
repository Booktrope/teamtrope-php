...
<?php 	
	...
	outputProjectHead($custom_fields, $this_id);
...
function outputProjectHead($custom_fields, $this_id) {
	if ( $custom_fields['book_ebook_front_cover'][0] != '' ) {
		echo '<img class="book-thumb alignright" width="130px" src="' . wp_get_attachment_url($custom_fields['book_ebook_front_cover'][0]) . '">';
	}	
	if ($custom_fields['book_author'][0] != "" ) {
		$sep = "";
		echo "<h4>by ";
		foreach ( unserialize($custom_fields['book_author'][0]) as $author ) {
			$user_info = get_userdata($author); 
			echo $sep . $user_info->display_name;
			$sep = ", ";
		}
		echo "</h4>";
	}
	...
	if ( isset($custom_fields['book_teamroom_link'][0]) && $custom_fields['book_teamroom_link'][0] != "" ) {
		echo "<br/><a href='" . $custom_fields['book_teamroom_link'][0] . "'>Link to Teamroom</a><br/>";
			...
			echo "<h4><a href='/groups/?tree'>Go to Teamrooms List</a></h4>\n\r";
	} 
}
...
?>
				...
				<label <?php echo $label_attr; ?>"><?php echo $task->post_title; ?><i class="<?php echo $task_custom['pcr_icon'][0]; ?>"></i></label>
<?php
			$j++;
		}
		return $tasks;
...
<?php	
	...
	function outputProjectDetails($task, $task_custom, $custom_fields) {
		...
		echo "<strong>Production:</strong> " . $custom_fields['book_pcr_step'][0] . " " ;
	...
	function outputAssets($task, $task_custom, $custom_fields) {
		...
		if (  in_array($pcr_name, array("Final Manuscript","Show All")) ) { 
		...
		if ( in_array($pcr_name, array("Final Page Count", "Approve Cover Art", "Upload Cover Templates", "Show All")) ) { 
		...
		if ( in_array($pcr_name, array("Final Page Count", "Upload Cover Concept", "Show All")) ) { 
		...
		if ( in_array($pcr_name, array("Upload Cover Concept", "Approve Cover Art", "Show All")) ) { 
		...
		if ( in_array($pcr_name, array("Final Page Count", "Upload Cover Concept", "Approve Cover Art", "Upload Cover Templates", "Publication Fact Sheet (PFS)", "Final Manuscript", "Published", "Show All")) ) { 
		...
		if ( in_array($pcr_name, array("Select Layout Style", "Layout Upload", "Approve Layout", "Final Page Count", "Upload Cover Concept", "Approve Cover Art", "Upload Cover Templates", "Publication Fact Sheet (PFS)", "Final Manuscript", "Show All")) ) { 
		...
		if ( in_array($pcr_name, array("Select Layout Style", "Layout Upload", "Approve Layout", "Final Page Count", "Upload Cover Concept", "Approve Cover Art", "Upload Cover Templates", "Publication Fact Sheet (PFS)", "Show All")) ) { 
		...
		if ( $pcr_name === "Submit Edited" ) {
			echo ' Edit Complete Date: ' . date('m-d-Y', strtotime($custom_fields['book_edit_complete_date'][0])) . "<br/>";
		} 
		if ( in_array($pcr_name, array("Edit Complete Date", "Submit Edit Complete Date", "Show All")) ) { // when to show Original Manuscript
			echo '<h4>Manuscript</h4>';
...
?>
		<h4>Three Cheers for the Team!</h4>
