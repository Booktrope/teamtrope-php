...
function bp_dtheme_add_custom_header_support() {
	...
	function bp_dtheme_header_style() { ?>
		<style type="text/css">
			<?php $header_edit_img_on = get_option('dev_studio_header_edit_img_on');
			if( $header_edit_img_on != 'no' ){
			?>
			#headercustom { height: 100px; background-image: url(<?php header_image() ?>); }
			<?php }else{ ?>
			#header-wrapper{background: url(<?php header_image() ?>); background-size: cover;}
			<?php } ?>
