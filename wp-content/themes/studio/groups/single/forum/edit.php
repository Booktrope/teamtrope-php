...
<?php if ( bp_has_forum_topic_posts() ) : ?>
	...
	<form action="<?php bp_forum_topic_action(); ?>" method="post" id="forum-topic-form" class="standard-form">
		...
		<div class="item-list-tabs" id="subnav" role="navigation">
			<ul>
				<li>
					<a href="#post-topic-reply"><?php _e( 'Reply', 'studio' ); ?></a>
				</li>
		...
		<div id="topic-meta">
			<h3><?php _e( 'Edit:', 'studio' ); ?> <?php bp_the_topic_title(); ?> (<?php bp_the_topic_total_post_count(); ?>)</h3>
		...
		<?php if ( bp_group_is_member() ) : ?>
			...
			<?php if ( bp_is_edit_topic() ) : ?>
				...
				<div id="edit-topic">
					...
					<label for="topic_title"><?php _e( 'Title:', 'studio' ); ?></label>
					<input type="text" name="topic_title" id="topic_title" value="<?php bp_the_topic_title(); ?>" />

					<label for="topic_text"><?php _e( 'Content:', 'studio' ); ?></label>
					...
					<p class="submit"><input type="submit" name="save_changes" id="save_changes" value="<?php _e( 'Save Changes', 'studio' ); ?>" /></p>
...
<?php else: ?>
	...
	<div id="message" class="info">
		<p><?php _e( 'This topic does not exist.', 'studio' ); ?></p>
	</div>
